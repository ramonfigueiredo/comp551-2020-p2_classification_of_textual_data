...
'''
def run_deep_learning_using_keras_with_layers_Embedding_BidirectionalLSTM_GlobalMaxPool1D_DenseRELU_Dropout_DenseSIGMOID(options):
    ...
    for dataset in dataset_list:
        ...
        loss, accuracy = model.evaluate(X_t, y, verbose=False)
        print(dataset)
        print("\tTraining accuracy score: {:.2f}%".format(accuracy*100))
        print("\tLoss: {:.4f}".format(loss))
        loss, accuracy = model.evaluate(X_te, y_test, verbose=False)
