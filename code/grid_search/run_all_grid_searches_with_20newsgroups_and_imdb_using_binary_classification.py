...
def run_classifier_grid_search(classifer, classifier_enum, param_grid, dataset, final_classification_table_default_parameters, final_classification_table_best_parameters):
    ...
    if dataset == Dataset.TWENTY_NEWS_GROUPS:
        ...
        X_train, y_train = \
            load_imdb_reviews(subset='train', binary_labels=True, verbose=False, shuffle=True, random_state=0,
                              db_parent_path=db_parent_path)
        X_test, y_test = \
            load_imdb_reviews(subset='test', binary_labels=True, verbose=False, shuffle=True, random_state=0,
                              db_parent_path=db_parent_path)
...
def run_grid_search(save_logs_in_file):
    if save_logs_in_file:
        if not os.path.exists('run_all_grid_searches_with_20newsgroups_and_imdb_using_binary_classification'):
            os.mkdir('run_all_grid_searches_with_20newsgroups_and_imdb_using_binary_classification')
        logging.basicConfig(filename='run_all_grid_searches_with_20newsgroups_and_imdb_using_binary_classification/all.log', format='%(asctime)s - %(levelname)s - %(message)s',
